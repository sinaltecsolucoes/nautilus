...
class PedidoController
{
    ...
    public function listarPedidos()
    {
        ...
        // Verifica permissão
        if (!PermissaoService::checarPermissao($cargo, 'Previsoes', 'Ler')) {
            // Limpa buffer e retorna erro JSON com código 403
        ...
        try {
            ...
            // 1. Limpa qualquer output anterior (espaços em branco de includes)
            if (ob_get_length()) ob_clean();

            // 2. Define explicitamente que é JSON
            header('Content-Type: application/json; charset=utf-8');

            // 3. Imprime o JSON
            echo json_encode($output);

            // 4. MATA o script aqui para evitar que HTML do layout seja adicionado no final
            exit;
        } catch (\Exception $e) {
